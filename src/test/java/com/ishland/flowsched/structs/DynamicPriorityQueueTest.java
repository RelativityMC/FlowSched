...
class DynamicPriorityQueueTest {
    ...
    private DynamicPriorityQueue<String> queue;

    @BeforeEach
    void setUp() {
        queue = new DynamicPriorityQueue<>(256);
        queue.enqueue("a", 5);
        queue.enqueue("b", 10);
        queue.enqueue("c", 20);
        queue.enqueue("d", 30);
    ...
    void testAssertions() {
        ...
        assertFalse(queue.changePriority("i", 0)); // attempt to change priority with non-existing element
        assertFalse(queue.changePriority("a", 5)); // attempt to change priority with same priority
    }

    private void assertQueue(String... expected) {
